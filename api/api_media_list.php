<?php
/* MEDIA LIST */
$app->get('/media(/:idMedia)', function ($idMedia = '') use ($app) {
	$get		= $app->request->get();
	...
	if (!checkTokenId($token, 'admin')) {
		...
		$whereId		= ($idMedia) ? " AND (id = '".$idMedia."')":"";
		$whereSearch	= ($search) ? " AND (filename LIKE '%".$search."%')":"";
		
		$sql		= "SELECT id, media,
						IF(media != '', CONCAT('".MEDIA_URL."', media), '') AS media_url
		...
		if ($result->num_rows == 0) {
			echo json_encode(array('status'=>'ERROR', 'message'=>'Media Not Found', 'error_code'=>'media_not_found', 'sql'=>preg_replace("/\s+/", " ", $sql)));
